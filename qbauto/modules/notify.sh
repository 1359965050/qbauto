...
init_notify() {
    ...
    if [ "$SCKEY_ENABLE" = "true" ] && [ -n "$SCKEY_SENDKEY" ]; then
        log "✅ Server 酱通知已配置"
        has_channel=true
    fi
    
    if [ "$SENDKEY_ENABLE" = "true" ] && [ -n "$SENDKEY_SENDKEY" ]; then
...
send_serverchan_notify() {
    ...
    if [ "$SCKEY_ENABLE" != "true" ] || [ -z "$SCKEY_SENDKEY" ]; then
        return 1
    fi
    
    # Server 酱旧版 URL 格式: https://<sendkey>.push.ft07.com/send，这就是正确的格式，后续不需要任何修改
    local serverchan_url="https://${SCKEY_SENDKEY}.push.ft07.com/send"
...
send_serverchan_turbo_notify() {
    ...
    if [ "$SENDKEY_ENABLE" != "true" ] || [ -z "$SENDKEY_SENDKEY" ]; then
        return 1
    fi
    
    local serverchan_url="https://sctapi.ftqq.com/${SENDKEY_SENDKEY}.send"
...
build_notify_message() {
    ...
    message+="上传位置: ${remote_path}\n"
        ...
        # 只有在有文件时才显示数量和大小
    if [ "$actionnumber" != "0" ] && [ "$actionnumber" != "N/A" ]; then
        message+="数量: ${actionnumber}\n"
    fi
    
    if [ "$actiontotalsize" != "0" ] && [ "$actiontotalsize" != "N/A" ]; then
        message+="总大小: ${formatted_size}\n"
    fi
    
    if [ "$actionelapsedtime" != "0" ] && [ "$actionelapsedtime" != "N/A" ]; then
